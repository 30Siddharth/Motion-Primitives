...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        self.graphpath = graph_search(world, self.resolution, self.margin, start, goal, astar=False)
        ...
        # self.K = self.get_coeff(self.Time_list,self.path)
    ...
    def update(self, t):
        ...
        #         c2 = c[:,2,:]
        #         c3 = c[:,3,:]
        #         x_ddot = np.concatenate(6*c3*t + 2*c2) #TODO: Coeff to be corrected
        ...
        for i in range(len(Time_list)-1):          
            if t == np.inf:
                ...
                c4 = c[:,4,:]
                c5 = c[:,5,:]
                #TODO: Coeff to be corrected # DONE 
                ''' The polynomial here is quintic'''
                x_ddddot = np.concatenate(120*c5*t + 24*c4)
                x_dddot  = np.concatenate( 60*c5*t**2 + 24*c4*t + 6*c3) 
                x_ddot   = np.concatenate( 20*c5*t**3 + 12*c4*t**2 + 6*c3*t + 2*c2) 
                x_dot    = np.concatenate( 5*t**4 + 4*c4*t**3 + 3*c3*t**2 + 2*c2*t + c1)
                x        = np.concatenate( c5*t**5 + c4*t**4 + c3*t**3 + c2*t**2 + c1*t + c0)
            elif Time_list[-1] <= t:
                x = path[-1]
            else:
                pass
    ...
    def get_coeff_5order(self,time,path):
        ...
        blks_vel = np.hstack((5*seg_time**4, 4*seg_time**3, 3*seg_time**2, 2*seg_time**1, seg_time**0, np.zeros(seg_time.shape)))
        vel_stack = np.zeros((3,blks_vel.shape[0],blks_vel.shape[1]))
        vel_stack[0,:,:] = blks_vel
        vel_stack[1,:,:] = blks_vel
        vel_stack[2,:,:] = blks_vel
        ...
        # print(Avel)
        Avel = Avel[:,1:-1,:]   # Becuase the first velocity does not coincide with anything
        # print(Avel)
        Avel = Avel[:,1::2,:] - Avel[:,0:-1:2,:]
        # print(Avel)
        # Avel[:,-1,1] = 1 
        ...
        blks_acc = np.hstack((20*seg_time**3,12*seg_time**2,6*seg_time**1,2*seg_time**0,np.zeros(seg_time.shape),np.zeros(seg_time.shape)))
        acc_stack = np.zeros((3,blks_acc.shape[0],blks_acc.shape[1]))
        acc_stack[0,:,:] = blks_acc
        acc_stack[1,:,:] = blks_acc
        acc_stack[2,:,:] = blks_acc
        ...
        blks_jerk = np.hstack((60*seg_time**2,24*seg_time**1,6*seg_time**0,np.zeros(seg_time.shape),np.zeros(seg_time.shape),np.zeros(seg_time.shape)))
        ...
        blks_snap = np.hstack((120*seg_time**1,24*seg_time**0,np.zeros(seg_time.shape),np.zeros(seg_time.shape),np.zeros(seg_time.shape),np.zeros(seg_time.shape)))
        ...
        A = np.concatenate((Apos,Avel,Aacc,Ajerk,Asnap,Avel_end,Aacc_end,Avel_start,Aacc_start),axis=1)
